...
//       户籍条款查不到时默认按"需广东省户籍"处理、履历字段移除

// ========== 学历等级 ==========
const DEGREE_RANK = { '大专': 1, '本科': 2, '硕士': 3, '博士': 4 };

// ========== 广东省 市→区县 映射（与 index.html cityDistricts 同步）==========
const CITY_DISTRICTS = {
  "广州": ["荔湾区", "越秀区", "海珠区", "天河区", "白云区", "黄埔区", "番禺区", "花都区", "南沙区", "从化区", "增城区"],
  "深圳": ["福田区", "罗湖区", "南山区", "宝安区", "龙岗区", "盐田区", "龙华区", "坪山区", "光明区", "大鹏新区", "深汕特别合作区"],
  "珠海": ["香洲区", "斗门区", "金湾区", "横琴粤澳深度合作区"],
  "佛山": ["禅城区", "南海区", "顺德区", "三水区", "高明区"],
  "惠州": ["惠城区", "惠阳区", "博罗县", "惠东县", "龙门县", "仲恺高新区", "大亚湾开发区"],
  "东莞": ["莞城街道", "南城街道", "东城街道", "万江街道", "石碣镇", "石龙镇", "茶山镇", "石排镇", "企石镇", "横沥镇", "桥头镇", "谢岗镇", "东坑镇", "常平镇", "寮步镇", "樟木头镇", "大朗镇", "黄江镇", "清溪镇", "塘厦镇", "凤岗镇", "大岭山镇", "长安镇", "虎门镇", "厚街镇", "沙田镇", "道滘镇", "洪梅镇", "麻涌镇", "中堂镇", "高埗镇", "松山湖", "滨海湾新区"],
  "中山": ["石岐街道", "东区街道", "西区街道", "南区街道", "五桂山街道", "火炬开发区", "民众街道", "南朗街道", "黄圃镇", "南头镇", "东凤镇", "阜沙镇", "小榄镇", "古镇镇", "横栏镇", "三角镇", "港口镇", "沙溪镇", "大涌镇", "板芙镇", "神湾镇", "三乡镇", "坦洲镇"],
...
function buildAbilitySet(profile) {
  ...
  //   10所重点师范=4, 华南师大=3.5, 广东省重点建设高校=3, 省属重点师范=3, 双非一本=2, 普通本科=1, 境外=-1单独标记
  function schoolLevel(s) {
    ...
    if (s.indexOf('华南师范') !== -1) return 3.5;
    if (s.indexOf('广东省重点建设高校') !== -1) return 3;
  ...
  const ab = {
    inGuangdong: profile.region !== 'other',
    city: profile.city || '',
    ...
    isGdKeySchool: st.indexOf('华南师范') !== -1 || st.indexOf('广东省重点建设高校') !== -1,
    ...
    bIsGdKeySchool: bSt.indexOf('华南师范') !== -1 || bSt.indexOf('广东省重点建设高校') !== -1,
    ...
    isFushuo: S('复硕'),
    isGuoyou: S('国优计划'),
    isElite: S('励儒卓越教师班') || S('卓越教师班') || S('精英班'),
...
function splitClauses(text) {
  ...
  // 再保护「学科并列」：如"音乐/体育/美术/信息科技"是同一句里的科目列举，不能被 '/' 拆成碎条款
  // （"985/211/双一流"这类院校并列不含学科词，仍按并列拆）
  const subjAlts = [];
  const masked2 = masked.replace(/[\u4e00-\u9fa5A-Za-z0-9]{2,8}(?:\/[\u4e00-\u9fa5A-Za-z0-9]{2,8}){1,8}/g, m => {
    if (/音乐|体育|美术|语文|数学|英语|物理|化学|生物|政治|历史|地理|科学|信息技术|信息科技|心理健康|道德与法治|思想政治|舞蹈|书法|劳动|特殊教育/.test(m)) {
      subjAlts.push(m); return `\u0003${subjAlts.length - 1}\u0004`;
    }
    return m;
  });
  const parts = masked2
  ...
  return parts.map(p => p
    .replace(/\u0001(\d+)\u0002/g, (_, i) => brackets[parseInt(i, 10)])
    .replace(/\u0003(\d+)\u0004/g, (_, i) => subjAlts[parseInt(i, 10)]));
}
...
function clauseHit(clause, ab) {
  ...
  // 拆 AND 组合（+ 且 并 及 同时）——但注意"华南师大广州大学"这类并列院校不拆
  // "县区级及以上""省级及以上前8名"里的"及"是固定搭配，先保护再拆，避免整句被误拆成 AND
  const cSafe = c.replace(/及以上/g, '\u0005');
  if (/[+＋且并及]/.test(cSafe) && !/师范生|技能竞赛|学科竞赛/.test(cSafe.replace(/[+＋且并及]/g, ''))) {
    const parts = cSafe.split(/[+＋且并及]/).map(s => s.trim().replace(/\u0005/g, '及以上')).filter(s => s.length >= 2);
    if (parts.length >= 2) {
      const hasStrictAnd = /且|并|及|同时/.test(cSafe);   // 明确"且/并/及" → AND
...
function singleClauseHit(c, ab) {
  ...
  // —— 明确"无荣誉门槛"类条款：直接通过，不判死 ——
  // （如"无统一荣誉门槛""无报考荣誉门槛""不限荣誉"；这类岗位荣誉只是加分项而非硬性资格）
  if (/无\s*(统一|报考)?\s*荣誉门槛|无统一荣誉|无报考荣誉|不限荣誉|无荣誉要求|不限制荣誉/.test(c)) return true;
  // 用户是否拥有任一"优秀毕业生/优秀称号"类荣誉（用于荣誉条款双向匹配）
  const hasYouxiu = ab.hasXiaoYouni || ab.hasShengYouni;
  // 院校限定词：条款含这些词时，"本科/硕士"应理解为"某类院校的本科/硕士"，走院校判定而非纯学历
  const hasSchoolWord = any(['双一流', '一流大学', '一流学科', '985', '211', '部属师范', '部属师范大学', '教育部直属', '华南师范', '华师', '广州大学', '深圳大学', '广东技术师范', '广东第二师范', '师范', '省属重点', '省重点', '重点建设', '重点师范', '高水平大学', '10所', '十所', '名校', 'C9', '九校', '清华', '北大', '境外', '国外', '海外', '留学', '世界前', '世界排名前', '部分高校']);
  ...
  const LGdKey = useBach ? ab.bIsGdKeySchool : ab.isGdKeySchool;
  ...
  if (any(['华南师范', '华师', '广州大学', '深圳大学', '广东技术师范', '广东第二师范'])) return LGdKey;
  if (any(['省属重点师范', '省重点师范', '省属重点建设高校', '重点建设高校', '高水平大学重点建设', '冲一流'])) return (useBach ? ab.bIsShengzhongdian || ab.bIsGdKeySchool : ab.isShengzhongdian || ab.isGdKeySchool);
  ...
  if (any(['励儒', '卓越教师班', '精英班', '拔尖创新人才', '荣誉学院'])) return ab.isElite;
  ...
  // 泛化兜底：岗位写"校级以上奖励/获得荣誉/表彰"等，用户有任一奖学金或优秀称号即满足
  if (/奖励|荣誉|表彰/.test(c)) return ab.hasGuojiang || ab.hasShengzhengFu || ab.hasSchoolFirst || ab.hasSchoolSecond || ab.hasSchoolThird || hasYouxiu;

  // —— 荣誉条款（优秀毕业生/称号）：双向匹配 ——
  // 用户填的是具体荣誉（如"校级优秀毕业生"），岗位写的是泛化要求（如"优秀毕业生"）；
  // 用户的较高级荣誉应当覆盖泛化要求，故两个方向都要认。
  if (/省优|省级优秀毕业生|省级优秀/.test(c)) return ab.hasShengYouni;          // 省级限定 → 需省级
  if (/优秀毕业生/.test(c)) return hasYouxiu;                                  // 泛化"优秀毕业生"→用户有任一优秀毕业生即满足
  if (/校优|校级优秀/.test(c)) return hasYouxiu;
  if (/优秀学生干部|优秀团干部|优秀学生|优秀团员|三好学生|优秀干部/.test(c)) return hasYouxiu || ab.hasSchoolFirst;
  // 兜底：含"优秀"且属荣誉/称号类（优秀毕业生/优秀教师/优秀班主任等），用户有任一优秀称号即满足
  if (/优秀/.test(c) && /荣誉|毕业生|称号|学生|干部|团员|教师|班主任/.test(c)) return hasYouxiu;
  ...
  // —— 户籍+排名组合（"广东户籍前40%"：广东户籍且排名达标）——
  // 注意："生源(可不受专业限制)"是括号豁免说明，不算独立资格条款；仅"广东户籍/生源"或"XX县户籍"才判
  const hasHukouClause = /户籍|生源/.test(c) && !/可不受|不受专业|放宽/.test(c);
  if (hasHukouClause) {
    // 精确户籍匹配：提取"XX市/县/区户籍"或"XX户籍"的地名，与用户 city/district 比对
    let hukouOK = ab.inGuangdong;   // 未写明地域默认广东省内
    ...
    if (mCity) {
      ...
      if (place === '广东' || place === '广东省') hukouOK = ab.inGuangdong;
      else hukouOK = ab.district.indexOf(place) !== -1 || ab.city.indexOf(place) !== -1;
    } else if (/广东省|广东/.test(c)) {
      hukouOK = ab.inGuangdong;
    }
    if (rankM) {
      const req = parseInt(rankM[1], 10);
...
function jobTargetYears(job) {
  ...
  // 标题中「20XX年招聘」也视为该届（如"2026年宁波市北仑区...招聘公告"）
  (text.match(/20(2[4-9])年/g) || []).forEach(s => years.add(parseInt(s.replace('年', ''), 10)));
...
const SUBJECT_EQUIV = {
  ...
  '道德与法治': ['道德与法治', '思想政治', '政治'],
  '思想政治': ['思想政治', '道德与法治'],
  // 特教岗专业口径宽：接受心理学类、教育康复学等（广东各地公告通行做法）
  '特殊教育': ['特殊教育', '心理健康'],
  '心理健康': ['心理健康', '特殊教育'],
  // 通用技术与信息技术同属技术类，广东小学「信息科技」与初中「信息技术」常合并招聘/互通
  '通用技术': ['通用技术', '信息技术'],
  '信息技术': ['信息技术', '通用技术'],
};
...
function normalizeJob(job) {
  ...
  else if (r.hukou === 'guangdong') hukouText = '广东省';
  else if (r.hukou === 'unknown') hukouText = '广东省（默认）';
  else if (r.hukou) {
    // 防御：拼音/非中文值显示成「广东省」而非裸拼音字符串
    if (!/[\u4e00-\u9fa5]/.test(r.hukou)) hukouText = '广东省（默认值异常）';
 ...
 *            region: 'guangdong'|'other', city, district, otherProvince,
...
function matchJobs(profile, jobs, options) {
  ...
  const inGuangdong = profile.region !== 'other';
  ...
  // 用户可报学科集合（教资学科 ∪ 专业学科），再按广东口径展开可通融学科
  ...
  return jobs.map(rawJob => {
    ...
    // 4. 户籍（广东绝大多数校招不限户籍；条款查不到 → 默认按"需广东省户籍"处理）
    ...
    if (hk === '不限') {
      ...
      if (inGuangdong) {
        score += 25;
        risks.push('户籍条款未查到，默认按需广东省户籍处理，请以原公告核实');
      } else {
        reasons.push('户籍条款未查到，默认需广东省户籍（省外户籍大概率不可报，请核实）');
      }
    } else if (hk === '广东省') {
      if (inGuangdong) {
        score += 30;
    ...
    // 6.3 师范类专业门槛（广东实际：本科通道部分限师范类，硕士通道多数不限）
    ...
    // 学科清单是否只是「摘要」（原文含 等/详见附件）→ 据「待核实」而非判不符
    // 广东公告常见写法："语文/数学/英语/物理…等多学科""详见岗位需求表"，此时不能因学科不对口判死
    const subjectsIncomplete = (stageInfo.reliable && !stageInfo.complete)
      || (j.rules && j.rules.subjectsIncomplete === true);
    ...
    if (negHit.length) {
      ...
      } else if (subjectsIncomplete) {
        // 公告只是摘要（含「等/约/其余/详见附件」），本学段岗位表可能未展开 → 提示查附件而非一刀切
        risks.push(`${stage}段公告列出的是${subjectsForMatch.length ? subjectsForMatch.join('、') : '未逐个列明学科'}，未见你的${Array.from(userSubjects).join('/')}岗位，请查附件岗位表确认`);
      } else {
    ...
    // 8.5 普通话检查已移除：广东教师招聘普通话要求为公开常识（语文二甲，其他学科二乙），
